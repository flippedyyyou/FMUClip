...
## 可尝试方案
+ 1. region-level loss不用sam3，用gradcam梯度法找对遗忘类最大的patch
+ 2. 同图保持背景，参考forget me not加噪思路，最小化对遗忘区域加噪后图像与遗忘区域交叉注意力降0后图像（解决text encoder自注意力机制导致其他prompt中残留遗忘类语义问题）
+ 3. coco classification改成开放词汇检测或图像分割任务？
+ 4. VQA任务参考FTTP或ICCV25细粒度知识编辑那篇，评测方法和benchmark。