...
class CLIPRewards(BaseRewards):
    ...
    @torch.no_grad()
    def extract_text_features(self, captions=None, tokenized_cap=None):
        if tokenized_cap is None and captions is None:
            raise ValueError("Either captions or tokenized_cap must be provided to extract_text_features")
        ...
        if tokenized_cap is not None:
            ...
            caption_tokens = tokenized_cap.to(self.device)
        else:
            caption_tokens = tokenize(captions).to(self.device)
