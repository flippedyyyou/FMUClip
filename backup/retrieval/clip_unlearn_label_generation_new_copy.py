...
def _load_forget_train_ids(dataset_train_ori, cfg, data_type):
    with open(f'/datanfs4/shenruoyan/FMUClip/Df/flickr/forget_dog_train.txt', 'r') as f:
...
def _load_forget_test_ids(dataset_test_ori, cfg, data_type):
    ...
    """
    with open(f'/datanfs4/shenruoyan/FMUClip/Df/coco/forget_dog_test.txt', 'r') as f:
        df_ids = [i.strip() for i in f.readlines() if i.strip()]
    ...
    if not use_original_only:
        ...
        if use_cliperase:
            ...
            supervised_unlearn_train(
                ...
                concept_token=getattr(args, "concept_token", "cat"),
