...
for DATASET in "flickr30k_entities"; do
  if [ "${DATASET}" = "coco2017_instances" ]; then
    ...
    FORGET_CLASSES="airplane,cow"
    TRAIN_ITEM_FOLDER="item5"
    RUN_NAME="airplane_cow_item5"
  elif [ "${DATASET}" = "flickr30k_entities" ]; then
    DF_ROOT="${FLICKR_DF_ROOT}"
    FORGET_CLASSES="girl,boy"
    TRAIN_ITEM_FOLDER="item4"
    RUN_NAME="girl_boy_item4"
  else
    echo "Unknown dataset: ${DATASET}"
    exit 1
  fi

  OUTPUT_DIR="finegrained/ckpt/vitb-32/${METHOD}_multi/${DATASET}/${RUN_NAME}/DF${LAMBDA_DF}_DR${LAMBDA_DR}_UNI${LAMBDA_UNI}"
