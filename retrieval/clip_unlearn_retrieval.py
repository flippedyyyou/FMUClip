    ...
    for ep in range(max_epoch):
        ...
        for it in range(iters_per_epoch):
            ...
            with torch.amp.autocast(device_type="cuda", enabled=True):
                ...
                patch_attn = _mask_to_patch_attention(sam3_masks, grid) # 形状为(B,N)的0/1矩阵
                attn_sum = patch_attn.sum(dim=1) # (B,)，mask=1 的有多少个 patch，保证不会因为 mask 大小影响 loss 尺度导致训练不稳
                ...
                loss = lambda_md * loss_attn + lambda_reward * loss_syn + lambda_uni * loss_uni
