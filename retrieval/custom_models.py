...
### PUT YOUR PATH HERE
DOWNLOAD_ROOT_v2 = '/datanfs4/shenruoyan/checkpoints/clip'
...
class CLIPRet(nn.Module):
    def __init__(self, device, arch, only_visual=True, momentum_update=False,
                    update_freq=256, update_w=1.0, momentum=0.9999):
        """
        Using CLIP to do image retrieval,
        only for reference.
        Args:
...
class CLIPRet_TTA(nn.Module):
    ...
    @torch.no_grad()
    def freeze_parameters(self):
        if self.only_visual: #retrieval_task==image2text
            for n, p in self.clip_model.named_parameters():
                if 'visual' not in n: #不是视觉编码器的参数不用回传梯度,logit_scale 也会被冻结
    ...
    def parameters(self, recurse: bool = True):
        if self.only_visual:
            ...
            for n, p in self.clip_model.named_parameters():
                if 'visual' not in n:
                #文本编码器的所有参数（Transformer、Embedding、LN、Projection 等）、logit_scale（如果有）
                    params.append(p) #优化器的 params 确实指向文本端参数
            return params 
